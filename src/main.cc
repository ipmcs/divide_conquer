...
void finish_warmup()
{
    ...
    for (uint32_t i=0; i<NUM_CPUS; i++) {
        ...
        ooo_cpu[i].l1i_prefetcher_reset();
...
void cpu_l1i_prefetcher_cache_fill(uint32_t cpu_num, uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint8_t evict_used)
{
  ooo_cpu[cpu_num].l1i_prefetcher_cache_fill(addr, set, way, prefetch, evicted_addr, evict_used);
}

void cpu_l1i_prefetcher_prefetch_response(uint32_t cpu_num, uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr)
{
  ooo_cpu[cpu_num].l1i_prefetcher_prefetch_response(addr, set, way, prefetch, evicted_addr);
}
...
int main(int argc, char** argv)
{
	...
	ooo_cpu[i].ITLB.MAX_READ = 2;
        ...
        ooo_cpu[i].L1I.l1i_prefetcher_prefetch_response = cpu_l1i_prefetcher_prefetch_response;
