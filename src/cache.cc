...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
    ...
    if (do_fill){
      ...
      if (cache_type == IS_L1I){
        l1i_prefetcher_cache_fill(fill_cpu, ((MSHR.entry[mshr_index].ip)>>LOG2_BLOCK_SIZE)<<LOG2_BLOCK_SIZE, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, ((block[set][way].ip)>>LOG2_BLOCK_SIZE)<<LOG2_BLOCK_SIZE, block[set][way].used);
        l1i_prefetcher_prefetch_response(fill_cpu, ((MSHR.entry[mshr_index].ip) >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, ((block[set][way].ip) >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE);
      }
      ...
      if (way >= 0) { // writeback hit (or RFO hit for L1D)
        ...
        if (cache_type == IS_L1D) { // RFO miss
          ...
          if (do_fill) {
            // update prefetcher
            if (cache_type == IS_L1I){
              l1i_prefetcher_cache_fill(writeback_cpu, ((WQ.entry[index].ip)>>LOG2_BLOCK_SIZE)<<LOG2_BLOCK_SIZE, set, way, 0, ((block[set][way].ip)>>LOG2_BLOCK_SIZE)<<LOG2_BLOCK_SIZE, block[set][way].used);
              l1i_prefetcher_prefetch_response(writeback_cpu, ((WQ.entry[index].ip) >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE, set, way, 0, ((block[set][way].ip) >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE);
            }
            if (cache_type == IS_L1D){
  ...
  void CACHE::handle_read()
  {
      ...
      for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
          ...
          if (way >= 0) { // prefetch hit
            // inform the prefetcher that requested prefetch is in the L1i
            if(cache_type == IS_L1I){
              l1i_prefetcher_prefetch_response(prefetch_cpu, ((PQ.entry[index].ip) >> LOG2_BLOCK_SIZE) << LOG2_BLOCK_SIZE, set, way, (PQ.entry[index].type == PREFETCH) ? 1 : 0, 0);
            }
    ...
    void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
    {
      ...
      block[set][way].used = (block[set][way].prefetch) ? 0 : 1;
