...
class O3_CPU {
  ...
  pair<int, uint64_t> l1i_prefetcher_branch_operate(uint64_t ip, uint8_t branch_type, uint64_t branch_target, uint8_t pTaken, uint8_t taken);
  ...
  void l1i_prefetcher_cache_fill(uint64_t v_addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_v_addr, uint8_t evict_used);
  void l1i_prefetcher_final_stats();
  int prefetch_code_line(uint64_t pf_v_addr); 
  void l1i_prefetcher_prefetch_response(uint64_t v_addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_v_addr);
  void l1i_prefetcher_reset();
};
